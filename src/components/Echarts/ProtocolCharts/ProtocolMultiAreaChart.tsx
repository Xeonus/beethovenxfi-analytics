...
interface ProtocolAreaChartProps {
    ...
    optimismProtocolData: ProtocolData,
}



export default function ProtocolMultiAreaChart({ftmProtocolData, optimismProtocolData}: ProtocolAreaChartProps) {

    const ftmData = ftmProtocolData.tvlData.map(el => Number(el.value.toFixed(2)));
    let opData = optimismProtocolData.tvlData.map(el => Number(el.value.toFixed(2)));
    ...
    const ftmxAxisData = ftmProtocolData.tvlData.map(el => el.time);
